...
export function updateAlgebraDayData(event: ethereum.Event): AlgebraDayData {
  ...
  if (algebraDayData === null) {
    ...
    algebraDayData.volumeMetis = ZERO_BD
...
export function updateFeeHourData(event: ethereum.Event, Fee: BigInt): void {
  ...
  let hourFeeID = event.address.toHexString().concat('-').concat(hourIndex.toString())
...
export function updateTokenDayData(token: Token, event: ethereum.Event): TokenDayData {
  ...
  let tokenPrice = token.derivedMetis.times(bundle.metisPriceUSD)
  ...
  tokenDayData.close = tokenPrice
  tokenDayData.priceUSD = token.derivedMetis.times(bundle.metisPriceUSD)
