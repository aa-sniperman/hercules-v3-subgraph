...
import { FACTORY_ADDRESS, ONE_BI, pools_list, ZERO_BD, ZERO_BI } from '../utils/constants'
import {
...
import { findNativePerToken, getNativePriceInUSD, getTrackedAmountUSD, priceToTokenPrices } from '../utils/pricing'
...
export function handleInitialize(event: Initialize): void {
  ...
  bundle.metisPriceUSD = getNativePriceInUSD()
  bundle.save()
  updatePoolDayData(event)
  updatePoolHourData(event)
  // update token prices
  token0.derivedMetis = findNativePerToken(token0 as Token)
  token1.derivedMetis = findNativePerToken(token1 as Token)
  token0.save()
  token1.save()
}
...
export function handleMint(event: MintEvent): void {
  ...
  let amountUSD = amount0
    .times(token0.derivedMetis.times(bundle.metisPriceUSD))
    .plus(amount1.times(token1.derivedMetis.times(bundle.metisPriceUSD)))

  // reset tvl aggregates until new amounts calculated
  factory.totalValueLockedMetis = factory.totalValueLockedMetis.minus(pool.totalValueLockedMetis)
  ...
  token0.totalValueLockedUSD = token0.totalValueLocked.times(token0.derivedMetis.times(bundle.metisPriceUSD))

  // update token1 data
  token1.txCount = token1.txCount.plus(ONE_BI)
  token1.totalValueLocked = token1.totalValueLocked.plus(amount1)
  token1.totalValueLockedUSD = token1.totalValueLocked.times(token1.derivedMetis.times(bundle.metisPriceUSD))
  ...
  pool.totalValueLockedMetis = pool.totalValueLockedToken0
    .times(token0.derivedMetis)
    .plus(pool.totalValueLockedToken1.times(token1.derivedMetis))
  pool.totalValueLockedUSD = pool.totalValueLockedMetis.times(bundle.metisPriceUSD)

  // reset aggregates with new amounts
  factory.totalValueLockedMetis = factory.totalValueLockedMetis.plus(pool.totalValueLockedMetis)
  factory.totalValueLockedUSD = factory.totalValueLockedMetis.times(bundle.metisPriceUSD)
  ...
  upsertTickFeeVars(lowerTick, event)
  upsertTickFeeVars(upperTick, event)
}
...
export function handleSwap(event: SwapEvent): void {
  ...
  if (amount0.lt(ZERO_BD)) {
    ...
    let communityFeeAmount = amount0.times(
      BigDecimal.fromString(pool.fee.times(pool.communityFee).toString()).div(BigDecimal.fromString('1000000000')),
    )
  ...
  let amount0Matic = amount0Abs.times(token0.derivedMetis)
  let amount1Matic = amount1Abs.times(token1.derivedMetis)

  let amount0USD = amount0Matic.times(bundle.metisPriceUSD)
  let amount1USD = amount1Matic.times(bundle.metisPriceUSD)
  ...
  let amountTotalMaticTracked = safeDiv(amountTotalUSDTracked, bundle.metisPriceUSD)
  ...
  // global updates
  factory.txCount = factory.txCount.plus(ONE_BI)
  factory.totalVolumeMetis = factory.totalVolumeMetis.plus(amountTotalMaticTracked)
  factory.totalVolumeUSD = factory.totalVolumeUSD.plus(amountTotalUSDTracked)
  factory.untrackedVolumeUSD = factory.untrackedVolumeUSD.plus(amountTotalUSDUntracked)
  factory.totalFeesMetis = factory.totalFeesMetis.plus(feesMatic)
  factory.totalFeesUSD = factory.totalFeesUSD.plus(feesUSD)

  // reset aggregate tvl before individual pool tvl updates
  let currentPoolTvlMatic = pool.totalValueLockedMetis
  factory.totalValueLockedMetis = factory.totalValueLockedMetis.minus(currentPoolTvlMatic)
  ...
  if (amount0.lt(ZERO_BD)) {
    ...
    poolDayData.feesToken1 = poolDayData.feesToken1.plus(
      amount1.times(pool.fee.toBigDecimal()).div(BigDecimal.fromString('1000000')),
    )
  }
  ...
  if (amount1.lt(ZERO_BD)) {
    ...
    poolDayData.feesToken0 = poolDayData.feesToken0.plus(
      amount0.times(pool.fee.toBigDecimal()).div(BigDecimal.fromString('1000000')),
    )
  }

  // update volume metrics
  algebraDayData.volumeMetis = algebraDayData.volumeMetis.plus(amountTotalMaticTracked)
  ...
  let numIters = oldTick.minus(newTick).abs().div(pool.tickSpacing)
...
function upsertTickFeeVars(tick: Tick, event: ethereum.Event): void {
...
export function handleChangeFee(event: ChangeFee): void {
  ...
  if (fee == null) {
    ...
    fee.fee = BigInt.fromI32(event.params.fee as i32)
